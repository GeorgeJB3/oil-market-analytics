...
def calculate_rolling_averages(df, commodities=gold_commodities):
    """
...
def prepare_correlations(df, commodities = ["brent_return", "wti_return", "natgas_return"], indices = ["sp500_return", "dxy_return"]):
    """
    """
    ...
    for commodity in commodities:
        for index in indices:
            ...
            df = df.withColumn(col_name, corr(commodity, index).over(rolling_30d))
    
    return df


def create_gold_oil_features(spark, req_cols):
    """
    """
    ...
    pivot_df = calculate_rolling_averages(pivot_df)
    ...
    gold_om_feat_df = prepare_correlations(joined_df)

    gold_om_feat_df = gold_om_feat_df.select(*req_cols).orderBy(desc("date"))

    try:
        gold_om_feat_df.write.mode("overwrite").saveAsTable(f"{SCHEMA_NAME}.{GOLD_TABLE_NAME}")
        print(f"Created gold table {SCHEMA_NAME}.{GOLD_TABLE_NAME}")
    except AnalysisException as ae:
        print(f"Analysis error when saving gold table {SCHEMA_NAME}.{GOLD_TABLE_NAME}: {ae}")
    except Exception as e:
        print(f"Unexpected error saving gold table {SCHEMA_NAME}.{GOLD_TABLE_NAME}: {e}")


def generate_gold_oil_features_table(spark):
    """
    Wrapper function to build gold oil features table.
    Invokes `create_gold_oil_features` function.

    Args:
        spark (SparkSession): Active Spark session.
    """
    create_gold_oil_features(spark, gold_columns)
